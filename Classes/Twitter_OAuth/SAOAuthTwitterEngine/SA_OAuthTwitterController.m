...
- (void) webViewDidFinishLoad: (UIWebView *) webView {
	...
	if(!authPin || [authPin isEqualToString:@""]) {
		authPin = [[_webView stringByEvaluatingJavaScriptFromString: @"document.getElementById('oauth_pin').getElementsByTagName(\"code\")[0].innerHTML"] stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceAndNewlineCharacterSet]];
	}
...
- (BOOL) webView: (UIWebView *) webView shouldStartLoadWithRequest: (NSURLRequest *) request navigationType: (UIWebViewNavigationType) navigationType {
  	...
  	if([request.URL.absoluteString rangeOfString:@"oauth_token"].location != NSNotFound && [request.URL.absoluteString rangeOfString:@"oauth_verifier"].location != NSNotFound)
    {
        [self gotPin: nil];
        return NO;
    }
